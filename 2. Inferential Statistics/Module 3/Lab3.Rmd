...
<li> <text style = "color:red">The births </text></li>
</ol>
...
<li> 0 </li>
<li> 13 </li>
<li> <text style = "color:red">27</text></li>
<li> 31 </li>
</ol>
...
<ol>
<li> Median birth weight of babies born to non-smoker mothers is slightly higher than that of babies
born to smoker mothers. </li>
<li> Range of birth weights of babies born to non-smoker mothers is greater than that of babies born to
smoker mothers. </li>
<li> <text style = "color:red">Both distributions are extremely right skewed.</test></li>
...
<div id="exercise">
**Exercise**: Are all conditions necessary for inference satisfied? Comment on each. You can compute the
group sizes using the same `by` command above but replacing `mean(weight)` with `n()`.
</div>
...
<li> $\color{red}{H_0: \mu_{smoking} = \mu_{non-smoking}}$;
$\color{red}{H_A: \mu_{smoking} \ne \mu_{non-smoking}}$ </li>
<li> $H_0: \bar{x}_{smoking} = \bar{x}_{non-smoking}$;$H_A: \bar{x}_{smoking} > \bar{x}_{non-smoking}$
</li>
<li> $H_0: \bar{x}_{smoking} = \bar{x}_{non-smoking}$; $H_A: \bar{x}_{smoking} > \bar{x}_{non-smoking}$
</li>
...
<ol>
<li> We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds
lighter at birth than babies born to smoker mothers. </li>
<li> We are 95% confident that the difference in average weights of babies whose moms are smokers and
nonsmokers is between 0.05 to 0.58 pounds. </li>
<li> We are 95% confident that the difference in average weights of babies in this sample whose moms are
smokers and nonsmokers is between 0.05 to 0.58 pounds. </li>
<li> <text style = "color:red">We are 95% confident that babies born to nonsmoker mothers are on average
0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers.</text></li>
</ol>

```{r habit-weight-ci}
# type your code for the Question 5 here, and Knit
t.test(nc$weight[nc$habit == 'nonsmoker'],nc$weight[nc$habit == 'smoker'],paired = F,
       var.equal = F,alternative = 'two.sided')$p.value  < 0.05
```
...
<li>(6.9779 , 7.2241)</li>
<li><text style = "color:red">(38.0952 , 38.5742)</text></li>
</ol>

```{r weeks-ci-99}
# type your code for Question 6 here, and Knit
mean(nc$weeks,na.rm = T) + c(1,-1) * qt(.005,sum(!is.na(nc$weeks)) - 1) * sd(nc$weeks,
    na.rm = T)/sqrt(sum(!is.na(nc$weeks)))
```

<div id="exercise">
**Exercise**: Calculate a new confidence interval for the same parameter at the 90% confidence level.
Comment on the width of this interval versus the one obtained in the the previous exercise.
</div>

```{r weeks-ci-90}
# type your code for the Exercise here, and Knit
mean(nc$weeks,na.rm = T) + c(-1,1) * qt(.95,sum(!is.na(nc$weeks)) - 1) * sd(nc$weeks,
    na.rm = T)/sqrt(sum(!is.na(nc$weeks)))
```

<div id="exercise">
**Exercise**: Conduct a hypothesis test evaluating whether the average weight gained by younger mothers
is different than the average weight gained by mature mothers.
</div>

```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(y = gained, x = mature, data = nc, statistic = "mean", type = "ht",alternative = 'twosided',
          method = "theoretical", order = c("younger mom","mature mom"))
```

7. Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.

<li><text style = "color:red">The maximum age of younger moms is 34 and minimum age of mature moms
is 35.</text></li>
<li>The maximum age of younger moms is 33 and minimum age of mature moms is 34.</li>
<li>The maximum age of younger moms is 32 and minimum age of mature moms is 33.</li>
<li>The maximum age of younger moms is 35 and minimum age of mature moms is 36.</li>

```{r cutoff-mature}
# type your code for Question 7 here, and Knit
younger <- nc$mage[nc$mature == "younger mom"]
younger <- younger[!is.na(younger)]
mature  <- nc$mage[nc$mature == "mature mom"]
mature  <- mature[!is.na(mature)]
summary(younger)[["Max."]]
summary(mature)[["Min."]]
```
