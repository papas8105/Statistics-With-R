...
* No flight is delayed more than 2 hours.  Correct
...
hist(sfo_feb_flights$arr_delay,col = 'red',breaks = 24)
```
...
* JetBlue Airways  
* Virgin America  
* Delta and United Airlines Correct 
* Frontier Airlines 
</div>

```{r sfo-feb-flights-arrival-delays-carrier}
# type your code for Question 3 here, and Knit
sfo_feb_flights %>% group_by(carrier) %>% summarise(IQR = quantile(probs = .75,arr_delay) - 
                                                          quantile(probs = .25,arr_delay),
...
* Median would be more reliable as the distribution of delays is skewed.  Correct
...
nycflights %>% select(tailnum,distance,air_time) %>% group_by(tailnum) %>%
  mutate(avg_speed = distance / (air_time * 60)) %>% arrange(desc(avg_speed)) -> cache_data
head(cache_data)
```
...
* There is an overall postive association between distance and average speed. Correct
...
plot(cache_data$distance,cache_data$avg_speed,col = rgb(1,0,0,.2),pch = 19,cex = 2)
```
...
`dep_type` with levels `"on time"` and `"delayed"` depending on the flight was delayed for fewer
than 5 minutes or 5 minutes or more, respectively. In other words, if `arr_delay` is 0 minutes or
fewer, `arr_type` is `"on time"`. If `dep_delay` is less than 5 minutes, `dep_type` is `"on
time"`. Then, determine the on time arrival percentage based on whether the flight departed on
time or not. What percent of flights that were `"delayed"` departing arrive `"on time"`?
...
## we defined dep_type in a previous question, answer in the first line
nycflights %>% group_by(dep_type) %>% summarise(percentage = sum(arr_type == "on time") / n()) 
```