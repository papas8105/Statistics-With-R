...
Our study is _observational_ and will be _generalizable_ to the public since the
sample is random and we consider there 
...
a one-against-all linear regression model and by subtracting one coefficient at a time with the biggest p-value 
over 0.15^[we don't need to strictly adjust strong regressors to avoid overfit] and stop at a step in which no other
...
from the F-statistic we deduce that the model is _statistically significant_ and 
observe that the model justifies our 
exploratory analysis findings, also the R squared is very high nearly equal to 90%
which is a good sign that most of the 
...
p-values, we used this metric because we aimed at extrapolation beyond the data time scope and tried to capture the
parameters that __mostly__ affect film preferences by the audiences in the rotten tomatoes web site. We didn't
conduct a causal analysis since we didn't randomize or blocked for a confounding factor we spotted in the plots
presented, our only goal was to create a prediction model based on _observational_ study. Ending our report, we
would like to point out that further study on data collection variables should be conducted, for example we might
need information on how the audience scoring is affected by a successful prequel of the movie rated or even add a 
