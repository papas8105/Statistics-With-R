...
p_rain_positive <- 0.8
p_rain_negative <- 0.3
p_positive_rain <- (p_rain_positive * p_rain) / (p_rain_positive * p_rain + p_rain_negative*(1-p_rain))
100 *round(p_positive_rain,3)
```
...
```{r q5a-solution}
p_biased <- c(.2,.4,.5,.6,.8)
prior_probabilities <- c(1/8,1/8,1/2,1/8,1/8) 
likelihood <- dbinom(3,3,p_biased) * prior_probabilities
sum(likelihood[4:5]/sum(likelihood))
```