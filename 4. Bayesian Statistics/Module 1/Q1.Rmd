...
question("Which of the following corresponds to the formula for the discrete form of Bayes’ Rule?",
         answer("$\\mathbb{P}\\left(A|B\\right)=\\frac{\\mathbb{P}\\left(B|A\\right)}{\\mathbb{P}(B)}$"),
         answer("$\\mathbb{P}\\left(A|B\\right)=\\frac{\\mathbb{P}\\left(B|A\\right)\\mathbb{P}(B)}{\\mathbb{P}(A)}$"),
         answer("$\\mathbb{P}\\left(A|B\\right)=\\mathbb{P}(A)\\mathbb{P}(B)$"),
         answer("$\\mathbb{P}\\left(A|B\\right)=\\frac{\\mathbb{P}\\left(B|A\\right)\\mathbb{P}(A)}{\\mathbb{P}(B)}$",
                correct = TRUE))
```
...
```{r q2a,exercise = TRUE}
## make computations here, store results in parameters: 
...
```{r q3a,exercise = TRUE,exercise.setup = "q2b"}
## make computations here, use the saved parameters from solution 2
...
question("Which of the following is the best Bayesian interpretation of the following statement: the probability of Liverpool 
         ...
         answer("We would be indifferent between winning $1 with probability 0.9 and winning
                $1 if Liverpool beats Swansea City.", correct = TRUE),
...
question("A clinical researcher wants to determine whether a new drug, RU-487, is more effective than RU-486.  
         ...
         answer("Yes, since the p-value under $\\mathcal{H}_{0}$ of no difference is approximately equal to 0.048,
...
```{r q12}
question("Which of the following corresponds to a Frequentist interpretation of the statement
         'the probability of rain tomorrow is 30 percent?'",
         answer("A degree of belief of 0.3, where 0 means rain is impossible and 1 means rain is certain."),
         answer("Under similar conditions, it has rained 30 percent of the time in the past."),
         answer("If we predicted rain tomorrow, we would be 30\\% confident in our prediction."),
         answer("If conditions identical to tomorrow occurred an infinite number of times, we would observe
         rain on 30 percent of those days.",correct = TRUE))
```

### 13. 

```{r q13}
question("Which of the following statements can be used to describe a 95 percent Frequentist confidence
         interval constructed based on a data from an experiment with sample of size n?",
         answer("If we ran the same experiment on new samples of size n an infinite number of times, 
                95 percent of our intervals generated in this way would contain the true value of $\\mu$.",correct = TRUE),
         answer("All values of $\\mu$ within the 95 percent confidence interval are equally likely"),
         answer("$\\mu$ is in this interval 95 percent of the time."),
         answer("The probability that $\\mu$ falls within the interval is 0.95."))
```

### 14.

```{r q14}
question("Hearing about your brilliant success in working with M&Ms, Mars Inc. transfers you over to the Skittles department. 
         They recently have had complaints about tropical Skittles being mixed in with original Skittles. 
         You decide to conduct a frequentist analysis. If the findings suggest than more than 1% of sampled supposedly
         original Skittles are actually tropical you will recommend action to be taken and the production process to be
         corrected.
         You will use a significance level of $\\alpha = 0.1$. You randomly sample 300 supposedly original skittles, 
         and you find that five of them are tropical. What should be the conclusion of your hypothesis test? Hint - 
         $\\mathcal{H}_{0}: p = 0.01$ and $\\mathcal{H}_{a}: p > 0.01$.",
         answer("Reject $\\mathcal{H}_{0}$, since the p-value is equal to 0.027, which is less than $\\alpha = 0.1$."),
         answer("Fail to reject $\\mathcal{H}_{0}$, since the p-value is equal to 0.101, 
                which is greater than $\\alpha = 0.1$."),
         answer("Fail to reject $\\mathcal{H}_{0}$, since the p-value is equal to 0.245, 
                which is greater than $\\alpha = 0.1$."),
         answer("Fail to reject $\\mathcal{H}_{0}$, since the p-value is equal to 0.184, 
                which is greater than $\\alpha = 0.1$.",
                correct = TRUE)
)
```
